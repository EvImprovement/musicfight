 ...
 * Grace period = 1.0 second (allows buffer/audio startup time).
...
export function calculateQuestionScore(elapsedTimeSeconds: number): {
  ...
  const gracePeriod = 1.0; // 1 second buffer delay before score starts decreasing
  ...
  // Minimum 10 points if answered within the 10 seconds
